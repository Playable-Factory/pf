...
var effekseer = function () {
  ...
  var _onRuntimeInitialized = function _onRuntimeInitialized() {
    ...
    Module._loadImage = function (path) {
      ...
      setTimeout(()=>{
        res.image = effect.context.lib[path];
        res.isLoaded = true;
        effect._update();
      }, 500);
    ...
    Module._loadBinary = function (path, isRequired) {
      ...
      setTimeout(()=>{
        let buffer = effect.context.lib[path];
        res.buffer = buffer;
        res.isLoaded = true;
        effect._update();
      }, 500);
