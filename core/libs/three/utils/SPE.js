...
SPE.shaderChunks = {
    ...
    uniforms: [
        ...
        'uniform float scale;',
        'uniform float textureFramesSeperate;',
        'uniform float textureRandomFrame;',
    ].join( '\n' ),
    ...
    floatOverLifetime: [
        ...
        '',
        '    for( int i = 0; i < VALUE_OVER_LIFETIME_LENGTH; ++i ) {',
        '       fIndex = float( i );',
        '       shouldApplyValue = when_gt( deltaAge, fIndex ) * when_le( deltaAge, fIndex + 1.0 );',
...
SPE.shaders = {
	vertex: [
        ...
        '        if ( textureFramesSeperate == 1.0 ) {',

        '           vec3 c2 = isAlive * getColorOverLifetime(',
		'               positionInTime,',
		'               unpackColor( color.x ),',
		'               unpackColor( color.y ),',
		'               unpackColor( color.z ),',
		'               unpackColor( color.w )',
		'           );',
        // '            frameNumber = textureRandomFrame;',
        '            frameNumber =   mod( round(maxAge * 40.0), totalFrames ) + 0.1;',
        // '            frameNumber =  mod( round(maxAge * 40.0), totalFrames );',
        // '            frameNumber = 8.1;',
        '        }',

		'        float column = floor(mod( frameNumber, framesX ));',
		'        float row = floor( frameNumber / framesY );',
		// '        float column = floor(mod( frameNumber, framesX ));',
...
SPE.Group = function( options ) {
    ...
    this.textureFramesSeperate = options.texture.useSeperate;
    
    let randomFrame = Math.floor(Math.random() * this.textureFrameCount);
    this.textureRandomFrame = 3
    ...
    this.uniforms = {
        ...
        textureFramesSeperate: {
            type: 'f',
            value: this.textureFramesSeperate ? 1.0 : 0.0
        },
        textureRandomFrame: {
            type: 'f',
            value: this.textureRandomFrame ? 1.0 : 0.0
        },
        fogColor: {
            type: 'c',
    ...
    // const gl = globals.threeRenderer.getContext();

    // // const glVertexShader = gl.createShader( gl.VERTEX_SHADER );

	// // gl.shaderSource( glVertexShader, this.material.vertexShader );
	// // gl.compileShader( glVertexShader );

    // // let asd = globals.threeRenderer.getContext().getShaderInfoLog(glVertexShader);
	...
	// gl.shaderSource( glFragmentShader, this.material.fragmentShader );
	// gl.compileShader( glFragmentShader );

    // let asd2 = globals.threeRenderer.getContext().getShaderInfoLog(glFragmentShader);

    // console.log('program');
    // const program = gl.createProgram();
    // console.log(program)
    // let asd3 = globals.threeRenderer.getContext().getProgramInfoLog(program);

    // console.log(asd3);
	// //return shader;

    // // console.log(THREE, THREE.WebGLShader);
    // // const glVertexShader = new THREE.WebGLShader( gl, gl.VERTEX_SHADER, this.material.vertexShader );
    // // const glFragmentShader = new THREE.WebGLShader( gl, gl.FRAGMENT_SHADER, this.material.fragmentShader );
...
SPE.Emitter.prototype.sendParticles = function( dt ) {
    'use strict';
