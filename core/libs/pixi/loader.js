...
let TextureAtlas;
let SkeletonJson;
let AtlasAttachmentLoader;

if (app.hasSpine) {
  import(/* webpackChunkName: "dynamic-modules" */'pixi-spine').then(spineModule => {
    TextureAtlas = spineModule.TextureAtlas;
  }).catch(err => {
    throw new Error(err);
  });

  import(/* webpackChunkName: "dynamic-modules" */'@pixi-spine/runtime-4.1').then(spineModule => {
