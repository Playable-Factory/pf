...
class PostProcessing {
	constructor() {
		data = app.data;
		main = app.main;
		this.game = globals.pixiApp;
		this.scene = globals.pixiScene;
	}

	//oyunda restart varsa çağırın
	reset() {
	...
	initLUT(textureSrc, lutPower) {
		...
		globals.pixiScene.filters = globals.pixiScene.filters || [];
		globals.pixiScene.filters.push(this.lutPass);
	...
	initBloom(intensity = 0.5) {
		...
		globals.pixiScene.filters = globals.pixiScene.filters || [];
		globals.pixiScene.filters.push(this.unreal_bloomPass);
	...
	initFilmShader(nIntensity, sIntensity, sCount, grayscale) {
		...
		globals.pixiScene.filters = globals.pixiScene.filters || [];
		globals.pixiScene.filters.push(this.filmPass);
	...
	initPixelate(pixelSize) {
		...
		globals.pixiScene.filters = globals.pixiScene.filters || [];
		globals.pixiScene.filters.push(this.pixelatePass);
	...
	initColorEdit(hue, saturation) {
		...
		globals.pixiScene.filters = globals.pixiScene.filters || [];
		globals.pixiScene.filters.push(this.colorEditPass);
	}
	...
	initVignette(offset, color, animateSpeed, animateRatio) {
		...
		globals.pixiScene.filters = globals.pixiScene.filters || [];
		globals.pixiScene.filters.push(this.vignettePass);
	}
	...
	initToonShader(outlineColor, edgeSens = 0.5) {
		...
		globals.pixiScene.filters = globals.pixiScene.filters || [];
		globals.pixiScene.filters.push(this.toonPass);
