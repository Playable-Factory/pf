	...
	constructor() {
		...
		const effectProps = [
			...
			},
			{
				name: "fromRight",
				getStart: (t) => {
					t.x = scene.lastWidth + t.width * 0.5;
					t.alpha = 1;
				},
				getEnd: (t) => {
					return {
			...
			},
			{
				name: "fromBottom",
				getStart: (t) => {
					t.y = scene.lastHeight + t.height * 0.5;
					t.alpha = 1;
				},
				getEnd: (t) => {
					return {
			...
			},
			{
				...
				getEnd: (t) => {
					return {
						y: () => scene.lastHeight + t.height * 0.5,
					};
				},
				ease: "back.in(1.3)",
			},
