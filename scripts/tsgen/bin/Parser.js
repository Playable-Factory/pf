...
class Parser {
	...
	createClass(doclet) {
		...
		if (doclet.classdesc) doclet.description = doclet.classdesc.replace(regexEndLine, "$1\n"); // make sure docs will be added
		return obj;
	}
	...
	createTypedef(doclet) {
		...
		if (doclet.type.names[0] === "object") {
			...
			for (let propDoc of doclet.properties) {
				...
				if (propDoc.description) prop.jsDocComment = propDoc.description.replace(regexEndLine, "$1\n");
			}
	...
	processFlags(doclet, obj) {
		...
		if (doclet.variable === true) {
			...
			if (!type.name.endsWith("[]")) {
				if (type.name != "any") console.log(`Warning: rest parameter should be an array type for ${doclet.longname}`);
